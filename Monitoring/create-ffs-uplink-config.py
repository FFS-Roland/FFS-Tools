...
def WriteNetworkFile(Segment,MonitorID):
    ...
    try:
        ...
        OutFile.write('    post-up     /usr/sbin/batctl -m $IFACE mm 0 || true\n')
        OutFile.write('    post-up     /usr/sbin/batctl -m $IFACE it 10000 || true\n')
        OutFile.write('    post-up     echo 255 > /sys/devices/virtual/net/$IFACE/mesh/hop_penalty || true\n\n')
        ...
        OutFile.write('    pre-up      /sbin/modprobe batman_adv\n')
        OutFile.write('    post-up     /usr/sbin/batctl -m bat%02d if add $IFACE\n' % (Segment))
        OutFile.write('    post-up     /sbin/ip link set dev bat%02d up\n' % (Segment))
        OutFile.write('    pre-down    /usr/sbin/batctl -m bat%02d if del $IFACE\n' % (Segment))
        OutFile.close()
    except:
